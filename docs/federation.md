...
This is a DGS example that demonstrates federation, the use of data loaders, codegen, and testing of federated queries.
...
This is configured in `build.gradle` as described in the [manual](https://manuals.netflix.net/view/dgs/mkdocs/master/generating-code-from-schema/).
When running `./gradlew build` the Java types are generated into the `build/generated` folder, which is automatically added to the classpath.

### Provide an Entity Fetcher

Imagine the following query sent to the gateway:
```graphql
query {
  shows {
    title
...
[Full code](https://stash.corp.netflix.com/projects/PX/repos/dgs-federation-example/browse/src/main/java/com/netflix/dgs/federation/example/datafetchers/FakeReviewDataFetcher.java)
```java
...
While you can always test federated queries by running the gateway and your DGS locally, it can be cumbersome and not ideal for setting up automated tests.
You can also test a federated query against just your DGS, without the gateway, using the `_entities` query to replicate the call made to your DGS by the gateway.
...
For more details non the API and how to set it up for tests, please refer to our documentation [here](./advanced/federated-testing.md#Using-the-Entities-Query-Builder-API).
